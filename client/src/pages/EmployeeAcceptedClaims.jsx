...
import { 
  Loader2, FileText, Download, CheckCircle, Car, Coffee, HelpCircle, CheckCircle2
} from 'lucide-react';

export default function EmployeeAcceptedClaims() {
  ...
  const fetchMyClaims = async () => {
    try {
      ...
      // Filter directly for accepted/approved claims
      const approvedOnly = res.data.filter(c => c.status === 'APPROVED');
    ...
    <div className="p-8 max-w-[1400px] mx-auto space-y-8 animate-in fade-in duration-500 min-h-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#003345] p-8 rounded-3xl relative overflow-hidden flex flex-col justify-center text-white h-56 shadow-sm">
           ...
           <h2 className="text-3xl font-headline font-black mb-3">Your Accepted Claims</h2>
           <p className="text-white/80 font-medium max-w-[85%] leading-relaxed">
             Track your finalized and approved reimbursements. These items have successfully passed all operational review tiers.
           </p>
           <HelpCircle className="w-48 h-48 absolute -right-12 -bottom-12 text-white/5" strokeWidth={1} />
        </div>

        {/* Total Processed Widget */}
        <div className="bg-green-50 p-8 rounded-3xl border border-green-100 flex flex-col justify-center h-56 shadow-sm">
           <p className="text-xs font-black tracking-widest text-green-800/60 uppercase mb-4">Total Value Reimbursed</p>
           <div className="flex items-end gap-3 mb-4">
              <h2 className="text-6xl font-headline font-black text-green-700 leading-none">
                ${totalApprovedAmount.toLocaleString('en-US', {minimumFractionDigits:2, maximumFractionDigits:2})}
              </h2>
           </div>
           <p className="text-sm font-bold text-green-800/70">Representing {claims.length} completely finalized documents</p>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-[#003345]/10 shadow-sm overflow-hidden flex flex-col mt-4">
        ...
        <div className="p-8 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-6">
           <div>
             <h3 className="text-2xl font-headline font-black text-[#003345] mb-1">Accepted Ledger</h3>
             <p className="text-[#40484c]/60 font-medium">Your successfully approved reimbursement claims.</p>
           </div>
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
             ...
             <tbody>
               {loading ? (
                   ...
                   <tr><td colSpan="5" className="p-16 text-center text-[#40484c]/60 font-medium text-lg">No accepted claims found yet.</td></tr>
               ) : (
                 currentItems.map((claim) => (
                     <tr key={claim.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                       <td className="px-8 py-6">
                         <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-[#003345]/5 flex items-center justify-center text-[#003345]">
                               {getCategoryIcon(claim.category)}
                            </div>
                            <span className="font-bold text-[#003345]">{claim.category || 'Standard'}</span>
                         </div>
                       </td>
                       <td className="px-6 py-6 font-medium text-sm text-[#40484c]">
                          {claim.description || 'No detailed description'}
                       </td>
        ...
        {totalPages > 1 && (
          <div className="p-4 border-t border-gray-100 flex items-center justify-between">
             <span className="text-sm font-medium text-[#40484c]/60">
                 Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, claims.length)} of {claims.length}
             </span>
             <div className="flex gap-2">
                 <button 
                   onClick={() => paginate(currentPage - 1)}
                   disabled={currentPage === 1}
                   className="px-3 py-1 rounded-lg text-sm font-bold text-[#003345] hover:bg-[#003345]/5 disabled:opacity-50"
                 >
                   Prev
                 </button>
                 <button 
